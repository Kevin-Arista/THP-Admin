...
export default function EditAllowedDomainPage() {
	...
	useEffect(() => {
		createClient().from("allowed_signup_domains").select("apex_domain").eq("id", id).single().then(({ data }) => {
			if (data) setApexDomain(data.apex_domain ?? "");
			setLoading(false);
		});
	}, [id]);
	...
	return (
		<div style={{ maxWidth: "420px" }}>
			<div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.75rem" }}>
				...
				<h1 style={{ fontSize: "1.4rem", fontWeight: 800, color: "#f0f0ff" }}>Edit Domain</h1>
			</div>
			<form onSubmit={handleSubmit} style={{ background: "#16213e", border: "1px solid #2a2a4a", borderRadius: "12px", padding: "1.75rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
					...
					<input type="text" value={apexDomain} onChange={(e) => setApexDomain(e.target.value)} required disabled={saving} style={inputStyle} />
				</div>
