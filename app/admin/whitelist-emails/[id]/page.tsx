...
export default function EditWhitelistEmailPage() {
	...
	useEffect(() => {
		createClient().from("whitelist_email_addresses").select("email_address").eq("id", id).single().then(({ data }) => {
			if (data) setEmailAddress(data.email_address ?? "");
			setLoading(false);
		});
	}, [id]);
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		const res = await fetch(`/api/admin/whitelist-emails/${id}`, {
			...
			body: JSON.stringify({ email_address: emailAddress.trim() }),
		});
		if (!res.ok) {
		...
		router.push("/admin/whitelist-emails");
		router.refresh();
	}
	...
	return (
		<div style={{ maxWidth: "420px" }}>
			<div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.75rem" }}>
				<Link href="/admin/whitelist-emails" style={{ color: "#5a5a7a", fontSize: "0.85rem" }}>← Whitelisted Emails</Link>
				<span style={{ color: "#2a2a4a" }}>/</span>
				<h1 style={{ fontSize: "1.4rem", fontWeight: 800, color: "#f0f0ff" }}>Edit Email</h1>
			</div>
			<form onSubmit={handleSubmit} style={{ background: "#16213e", border: "1px solid #2a2a4a", borderRadius: "12px", padding: "1.75rem", display: "flex", flexDirection: "column", gap: "1.25rem" }}>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
					<label style={{ fontSize: "0.75rem", fontWeight: 600, color: "#8888aa", letterSpacing: "0.05em" }}>Email Address *</label>
					<input type="email" value={emailAddress} onChange={(e) => setEmailAddress(e.target.value)} required disabled={saving} style={inputStyle} />
				</div>
				<div style={{ display: "flex", gap: "0.75rem" }}>
					...
					<Link href="/admin/whitelist-emails" style={{ padding: "0.65rem 1.25rem", fontSize: "0.9rem", color: "#8888aa", border: "1px solid #2a2a4a", borderRadius: "8px", textAlign: "center" }}>Cancel</Link>
				</div>
				<button type="button" onClick={handleDelete} disabled={deleting || saving} style={{ padding: "0.5rem", fontSize: "0.85rem", color: deleting ? "#5a5a7a" : "#ff6b6b", background: "transparent", border: "1px solid rgba(255,107,107,0.3)", borderRadius: "8px", cursor: deleting ? "default" : "pointer" }}>
					{deleting ? "Removing…" : "Remove from Whitelist"}
				</button>
